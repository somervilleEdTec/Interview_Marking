...
| -------------------------------------------------- | --------------------------------------- | ------------------------------------------- |
| Global shortcuts A S D F J K L ; Space N Backspace | Marking started                         | Marks fire; UI flash/ribbon update          |
| DualSense face + LB/LT/RB/RT                       | Marking started                         | Marks fire                                  |
| DualSense idle ~10+ min                            | Keepalive poll                          | Status shows disconnect; reconnect recovers |
| Shortcuts while **stopped**                        | —                                       | No global capture                           |
...
Renderer polls `navigator.getGamepads()` every 32ms while marking is started and forwards edge presses to the main process. Connection state is shown in the top bar (`#pad-status`).
